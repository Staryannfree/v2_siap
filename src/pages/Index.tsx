...
const Index = () => {
  ...
  useEffect(() => {
    if (typeof chrome !== 'undefined' && chrome.tabs) {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        const validPage = currentUrl.includes('siap.educacao.go.gov.br');
        setIsSiapPage(validPage);
      });
    } else {
      setIsSiapPage(true); // Fallback para dev/localhost
    }
  }, []);
  ...
  const tourSteps: any[] = [
    ...
    },
    {
      ...
      content: 'Esta é a barra de funções. No Planejamento Turbo, focamos no que importa: Frequência, Conteúdo e Planejamento.',
    },
    {
  ...
  const handleParearCelular = useCallback(async () => {
    ...
    const uid = session?.user?.id;
    if (!uid) {
      toast.error("Faça login na conta Planejamento Turbo para registrar o pareamento.");
      return;
    }
    const chromeApi = (window as any).chrome;
    let nome = "";
  ...
  if (!isSupabaseConfigured || !supabase) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 p-8 text-center">
        <img src={typeof chrome !== 'undefined' && chrome.runtime ? chrome.runtime.getURL('icon-48.png') : '/icon-48.png'} className="h-12 w-12 object-contain mb-4" alt="Planejamento Turbo" />
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 p-6">
        <div className="mb-8 flex flex-col items-center gap-2">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-600 shadow-lg shadow-indigo-900/40">
            <img src={typeof chrome !== 'undefined' && chrome.runtime ? chrome.runtime.getURL('icon-48.png') : '/icon-48.png'} className="h-8 w-8 object-contain" alt="Planejamento Turbo" />
          </div>
          <h1 className="text-xl font-black tracking-tight text-white">Planejamento Turbo</h1>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-slate-950 to-slate-900 p-6">
        <Card className="w-full max-w-md border-amber-500/40 bg-slate-900/50 shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-slate-400">
              Renove ou adquira sua assinatura para usar o Planejamento Mobile e o controle remoto.
            </CardDescription>
          </CardHeader>
      ...
      <div className="h-screen bg-gradient-to-b from-slate-50 to-white p-6 flex flex-col items-center justify-center text-center space-y-6 animate-in fade-in zoom-in-95 duration-700">
        <div className="space-y-4">
          <div className="w-20 h-20 bg-gradient-to-tr from-indigo-100 to-indigo-50 rounded-[1.5rem] flex items-center justify-center mx-auto shadow-xl border-2 border-white rotate-3">
            <img src={typeof chrome !== 'undefined' && chrome.runtime ? chrome.runtime.getURL('icon-48.png') : '/icon-48.png'} className="w-10 h-10 object-contain" alt="Planejamento Turbo" />
          </div>
          <div className="space-y-2">
            <h1 className="text-2xl font-black tracking-tight text-slate-950">Planejamento Turbo</h1>
  ...
  if (isSiapPage === null) {
    return (
      <div className="flex items-center justify-center h-screen bg-slate-50">
        <span className="text-slate-500 animate-pulse text-sm font-medium">Verificando ambiente...</span>
      </div>
    );
  }
  ...
  if (!isSiapPage) {
    return (
      <div className="flex flex-col items-center justify-center h-screen p-6 text-center bg-slate-50">
        <div className="w-16 h-16 mb-4 text-indigo-500 bg-indigo-100 rounded-full flex items-center justify-center">
          <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
          </svg>
        </div>
        <h2 className="text-xl font-bold text-slate-800 mb-2">Ambiente Incorreto</h2>
        <p className="text-sm text-slate-600 mb-6">
          O Lançamento Turbo só funciona dentro do portal oficial do SIAP.
        </p>
        <button 
          onClick={() => chrome.tabs.create({ url: 'https://siap.educacao.go.gov.br/' })}
          className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl shadow-sm transition-colors duration-200"
        >
          Abrir Portal SIAP
        </button>
      </div>
      ...
      <header className="w-full mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100 overflow-hidden">
            <img src={typeof chrome !== 'undefined' && chrome.runtime ? chrome.runtime.getURL('icon-48.png') : '/icon-48.png'} className="w-6 h-6 object-contain" alt="Planejamento Turbo" />
          </div>
          <div>
            <h1 className="text-sm font-black text-slate-950 tracking-tight leading-none mb-0.5">Planejamento Turbo</h1>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">Simples. Rápido. Automático.</p>
